...
namespace myComp {
  ...
  void Scanner::match(TokenType expected, const char *name) {
    if (this->token.type != expected) {
      std::cerr << "Syntax error: expected " << name << " but got token "
          << token_str.at(this->token.type) << " on line " << this->line
          << '\n';
      exit(1);
    }
    // Get the next token
  ...
  bool Scanner::is_data_type() const {
    return data_types.contains(this->token.type);
  }
