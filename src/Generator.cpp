...
namespace myComp {
    ...
    void Generator::generate(const ASTNode *node) {
        if (node->type == ASTNodeType::FUNCTION_DECLARATION)
            ...
            Errors::fatal_error("got unexpected AST node type " + std::string(ASTNode_str.at(node->type)));
    }
    ...
    int Generator::expression(const ASTNode *node) {
        ...
        switch (node_type) {
            ...
            case ASTNodeType::DEREFERENCE:
                if (node->right == nullptr)
                    return asm_generator->dereference(Generator::expression(node->left), node->data_type);
                else
                    return asm_generator->dereference(Generator::expression(node->left),
                                                      Generator::expression(node->right), node->data_type);
        ...
        switch (node_type) {
            ...
            case ASTNodeType::WIDEN:
                return asm_generator->widen_integer(left_reg, node->left->data_type, node->data_type);
    ...
    int Generator::assign(const ASTNode *node) {
        if (node->left->type == ASTNodeType::VARIABLE)
            return asm_generator->assign(node->left->string, Generator::expression(node->right));
