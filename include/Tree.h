...
namespace myComp {
    class Tree {
    public:
        static ASTNode *int_literal(long long value);

        static ASTNode *join(ASTNodeType type, ASTNode *left, ASTNode *right);

        static ASTNode *unary(ASTNodeType type, ASTNode *left);

        static ASTNode *variable_declaration(const std::string &identifier,
                                             Type_ *data_type);

        static ASTNode *function_declaration(std::string identifier,
                                             ASTNode *code_block);

        static ASTNode *function_call(const std::string &identifier,
                                      ASTNode *parameters);
