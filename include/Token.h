...
namespace myComp {
    ...
    public:
        Token(TokenType type, long long integer_val, std::string string_val)
                : type_(type), integer_val_(integer_val), string_val_(std::move(string_val)) {
        }

        // Token() : type_(TokenType::T_EOF), integer_val_(0) {
        // }
    ...
    private:
        ...
        static Token *getOrCreate(TokenType type, long long integer_val, const std::string &string_val) {
            ...
            if (const auto it = getCache().find(str); it != getCache().end())
                return &it->second;
            if (type == TokenType::INT_LITERAL)
                return &getCache().emplace(str, Token(type, integer_val, "")).first->second;
            if (type == TokenType::STRING_LITERAL || type == TokenType::IDENTIFIER)
                return &getCache().emplace(str, Token(type, 0, string_val)).first->second;
